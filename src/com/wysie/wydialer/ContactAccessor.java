...
final class ContactAccessor 
{
	...
	private static final String[] PHONE_PROJECTION 
	    = new String[] {Phone._ID, Phone.NUMBER, Phone.IS_PRIMARY };
	...
	class MyContactSplit implements IContactSplit 
	{
		...
		public Uri getCallUri(Uri lookupUri) 
		{
			...
			try 
			{
				...
				cursor.moveToFirst();
				Log.i("getCallUri", String.format("number: %s  pri: %d", 
												   cursor.getString(1),
												   cursor.getInt(2)));
	...
	private final static String peopleSql = 
		"(" + upName + " GLOB ? OR " 
		+ upName + " GLOB ? OR REPLACE(" + Phone.NUMBER + ",'-', '') GLOB ?) AND (is_primary = 1)";
